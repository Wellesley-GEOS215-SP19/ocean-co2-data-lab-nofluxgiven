...
%Create empty 3-dimensional arrays of NaN values to hold your reshaped data
    ...
    %temperature)
llmGrid = NaN*zeros(length(latgrid),length(longrid),length(mongrid));
pCO2Grid = NaN*zeros(length(latgrid),length(longrid),length(mongrid));
sstGrid = NaN*zeros(length(latgrid),length(longrid),length(mongrid));
%<--
